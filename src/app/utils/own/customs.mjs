 ...
 */
export function toastBeforeAddRecord(msg = null, button = null) {
    const span = msg ?? 'Should <span class="text-warning">fill</span> the form';
    const btn = button ?? addButton;
    Toast.show({ message: `${span} before press the button <button class="btn btn-primary">${btn}</button>`, mode: 'warning', duration: 1000 });
}
